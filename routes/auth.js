...
const sendMail = async (name, email, id) => {
  try {
    ...
    const mailOption = {
      ...
      html: `<p> hii  ${name} plz click here to <a href="http://127.0.0.1:1000/verify?id=${id}">Verify your Account</a> your mail </p>`,
    };
...
const sendResetPasswordMail = async (name, email, token) => {
  try {
    ...
    const mailOption = {
      ...
      html: `<p> hii  ${name} plz click here to <a href="http://127.0.0.1:1000/forget-password?token=${token}">Reset Password</a> your mail</p>`,
    };
    transporter.sendMail(mailOption, function (err, info) {
      if (err) {
        console.log(err);
...
router.post("/login", async (req, res) => {
  ...
  if (validPassword) {
    ...
    if (user?.isVerified=== true) {  
      ...
      return res.render("login", { message: "Please first verify your account" });
    }
  } else {
    return res.redirect("/login?message=User does not exist");
  }
...
router.post("/register", async (req, res) => {
  ...
  res.render("login",{message:"Your Account is being Registered Plz first Check your Mail and verify it"});
...
router.get("/verify", async (req, res) => {
  veryMail(req);
  res.render("verified_mail",{layout:"layouts/layout2.ejs"});
});
...
const pToken = async (req, res) => {
  try {
    ...
    if (tokenData?._id) {
      // console.log(tokenData._id)
      // console.log("hi");
      res.render("forget-password", {id:tokenData._id, layout:"layouts/layout2.ejs",message:""});
    } else {
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (passwordMatch) {
      res.render("forget-password",{message:"same password plz try different one"});
    } else {
      const salt = await bcrypt.genSalt(10);
      ...
      res.render("updated",{layout:"layouts/layout2.ejs"});;
    }
  } catch (error) {
    console.error(error);
